...
```
.
├── src/Compendium.Adapters.AzureOpenai/        — the adapter project (rename AzureOpenai → <Vendor>)
│   ├── DependencyInjection/
│   │   └── ServiceCollectionExtensions.cs
│   ├── Options/AzureOpenaiOptions.cs
│   └── AzureOpenaiAdapter.cs                   — illustrates the IAdapter (or any port) shape
├── tests/Unit/Compendium.Adapters.AzureOpenai.Tests/
│   ├── DependencyInjection/ServiceCollectionExtensionsTests.cs
│   ├── Options/AzureOpenaiOptionsTests.cs
│   └── GlobalUsings.cs
...
├── Compendium.Adapters.AzureOpenai.sln
...
find . -type f \( -name '*.cs' -o -name '*.csproj' -o -name '*.sln' -o -name '*.md' -o -name '*.yml' -o -name '*.json' -o -name '*.props' \) -exec sed -i '' -e "s/AzureOpenai/${VENDOR}/g" -e "s/azure-openai/${VENDOR,,}/g" {} +

# 4. Rename folders/files
git mv src/Compendium.Adapters.AzureOpenai              src/Compendium.Adapters.${VENDOR}
git mv src/Compendium.Adapters.${VENDOR}/Compendium.Adapters.AzureOpenai.csproj \
       src/Compendium.Adapters.${VENDOR}/Compendium.Adapters.${VENDOR}.csproj
git mv src/Compendium.Adapters.${VENDOR}/AzureOpenaiAdapter.cs                   \
       src/Compendium.Adapters.${VENDOR}/${VENDOR}Adapter.cs
git mv src/Compendium.Adapters.${VENDOR}/Options/AzureOpenaiOptions.cs           \
       src/Compendium.Adapters.${VENDOR}/Options/${VENDOR}Options.cs

git mv tests/Unit/Compendium.Adapters.AzureOpenai.Tests           tests/Unit/Compendium.Adapters.${VENDOR}.Tests
git mv tests/Unit/Compendium.Adapters.${VENDOR}.Tests/Compendium.Adapters.AzureOpenai.Tests.csproj \
       tests/Unit/Compendium.Adapters.${VENDOR}.Tests/Compendium.Adapters.${VENDOR}.Tests.csproj
git mv tests/Unit/Compendium.Adapters.${VENDOR}.Tests/Options/AzureOpenaiOptionsTests.cs \
       tests/Unit/Compendium.Adapters.${VENDOR}.Tests/Options/${VENDOR}OptionsTests.cs

mv Compendium.Adapters.AzureOpenai.sln Compendium.Adapters.${VENDOR}.sln
...
- **Author the actual adapter code.** Replace `AzureOpenaiAdapter` with the real implementation of whatever port (`IEventStore`, `IIdentityProvider`, `IBillingProvider`, `IEmailSender`, …) you're filling.
